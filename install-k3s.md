...
> **Objectif** : document de référence pour un **assistant IA** ou un nouveau contributeur travaillant **dans ce dépôt app** (hors mono-repo `k3s-homelab`). Dernière génération / mise à jour : **2026-03-27T19:47:29Z**.
...
| Paramètre | Détail |
|-----------|--------|
| **Label workflow** | `runs-on: arc-runner-casadelsabor` (nom du *scale set* Helm = nom d’installation ARC) |
...
| **Registry** | **GHCR** — image **`ghcr.io/vinzlac/casadelsabor`** (tags **`:<sha>`** et **`:main`**) |
...
| Paramètre | Valeur |
|-----------|--------|
| **Dépôt source (celui-ci)** | https://github.com/vinzlac/casadelsabor.git |
| **Branche** | main |
| **Chemin manifests** | `kubernetes/` |
| **Namespace cible** | **`casadelsabor`** |
| **Application Argo (nom)** | **`casadelsabor`** |
| **Déclaration côté pilote** | Fichier Application dans le repo **`https://github.com/vinzlac/k3s-homelab.git`** : `kubernetes/argocd/applications/casadelsabor.yaml` |
...
| Élément | Valeur |
|---------|--------|
| **Ingress (hostname)** | **`casadelsabor.homelab`** |
| **URL** | `http://casadelsabor.homelab` (Traefik, classe `traefik`) |
...
**Secret pull GHCR** (si le package image est **privé**) : secret **`ghcr-pull`** dans le namespace **`casadelsabor`**, référencé par le Deployment (`imagePullSecrets`).
...
| Clé | Valeur |
|-----|--------|
| **Org / repo GitHub** | vinzlac / **casadelsabor** |
| **Ressource K8s (Deployment/Service)** | casadelsabor |
| **Image** | `ghcr.io/vinzlac/casadelsabor:<sha>` (mis à jour par CI) |
