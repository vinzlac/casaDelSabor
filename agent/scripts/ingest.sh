...
if [ -f .env.local ] && grep -q "^API_KEY=" .env.local 2>/dev/null; then
    ...
    curl -s -X POST "http://localhost:${PORT}/ingest" \
        -H "X-API-Key: $API_KEY" \
        -H "Content-Type: application/json" | uv run python -m json.tool --no-ensure-ascii
    ...
    curl -s -X POST "http://localhost:${PORT}/ingest" | uv run python -m json.tool --no-ensure-ascii
fi
