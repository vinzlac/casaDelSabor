#!/usr/bin/env bash
# SealedSecret pour cette app (namespace casadelsabor) : une variable à la fois.
# Le nom du Secret est « casadelsabor-<suffixe> » ; le suffixe est dérivé du nom de la variable
# (ex. API_KEY → api → casadelsabor-api ; LLM_API_KEY → llm → casadelsabor-llm), sauf si --nature est fourni.
#
